...
def adjust_jump(coarse_current_dir,restrict_fine_bar_dir,coarse_bar_dir_last,ts_tpl,time_slice, target_coarse_dir):
    already_created = False
    try:
        os.mkdir(target_coarse_dir)
        print("Directory created "+ target_coarse_dir)
    except FileExistsError as fe:
        print(target_coarse_dir + " already exist")
        already_created = True
    
    if not already_created :
        utf.setup_coarse(opt.src_case_coarse,target_coarse_dir)
        utf.modify_control_param(target_coarse_dir,{"startTime":ts_tpl[time_slice][0],"endTime":ts_tpl[time_slice][1]})
        try:
            shutil.rmtree(target_coarse_dir+"/0")
        except FileNotFoundError as fn:
            print(target_coarse_dir+"/0"+" alredy deleted")
    
    target_time_dir = ts_tpl[time_slice][0]

    src = coarse_current_dir +'/'+str(target_time_dir)
    dst = target_coarse_dir  +'/'+str(target_time_dir)
    ...
    for f in files:
        ...
        coarse_current = dst + '/' + f
        r_fine_bar = restrict_fine_bar_dir+'/'+str(target_time_dir)+ '/' + f
        coarse_last = coarse_bar_dir_last+'/'+str(target_time_dir)+ '/' + f

        target_file = ppf(coarse_current)
        x = target_file.getValueDict()
        y = ppf(r_fine_bar).getValueDict()
        z = ppf(coarse_last).getValueDict()
